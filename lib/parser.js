...
var hasPath        = has('path');
var hasScope       = has('scope');
...
function _mapSymbol(symbol){
  ...
  var path        = symbol.tags.filter(hasPath);
  var scope       = symbol.tags.filter(hasScope);
  ...
  if(path.length === 1){
    symbol.path = path[0].string;
  }

  if(scope.length === 1){
    symbol.scope = scope[0].string;
  }
